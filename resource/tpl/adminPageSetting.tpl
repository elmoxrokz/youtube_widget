<form class="youtubewidgetSave" id="youtubewidgetSave" name="youtubewidgetSave" method="post">	
			...
			<tr>
				...
				<!-- {foreach from=$aOrder item=order name=olist}-->
				<td>
					<div class="content">
					...
					<div class="content">
						<select title="select tab" class="tab" id="show_html_value" size="4">
							<?php foreach($aOrder as $key => $val){ ?>
								<option id="optyoutubewidget_<?php echo $key;?>" value="<?php echo $val[alt];?>" >
									<span id="spanyoutubewidget_<?php echo $key;?>" class="taborder_lbl"><?php echo $aOrder[$key][text];?></span>
								</option>	
							<?php }?>
						</select>
					</div>
